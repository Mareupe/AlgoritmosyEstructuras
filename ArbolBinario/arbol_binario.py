...
class Arbol(object):
    ...
    def inorden_nombreC(self):
        if(self.info is not None):
            if(self.izq is not None):
                self.izq.inorden_nombreC()
            if(self.info[0]=='C'):
                print(self.info, self.datos)
            if(self.der is not None):
    ...
    def busqueda_Talos(self, clave):
        pos = None
        if(self.info is not None):
            if(self.info == clave):
                pos = self.datos
            elif(clave < self.info and self.izq is not None):
                pos = self.izq.busqueda_Talos(clave)
            elif(self.der is not None):
                pos = self.der.busqueda_Talos(clave)
        return pos
    ...
    def contar_heroes(self, buscado):
        cantidad = 0
        if(self.info is not None):
            if(self.datos['heroe'] == buscado):
                cantidad += 1
            if(self.izq is not None):
    ...
    def mostrar_criaturas_derrotadas(self, buscado):
        if(self.info is not None):
            if(self.datos['capturada'] == buscado):
                print(self.datos)
            if(self.izq is not None):
                self.izq.mostrar_criaturas_derrotadas(buscado)
            if(self.der is not None):
                self.der.mostrar_criaturas_derrotadas(buscado)
    ...
    def separar_arbol_Heroe_Villano(self, arbol_aux, categoria):
        if(self.info is not None):
            if (self.datos['heroe'] == categoria):
                arbol_aux = arbol_aux.insertar_nodo(self.info, self.datos)
            if(self.izq is not None):
                arbol_aux = self.izq.separar_arbol_Heroe_Villano(arbol_aux, categoria)
            if(self.der is not None):
               arbol_aux = self.der.separar_arbol_Heroe_Villano(arbol_aux, categoria)
        return arbol_aux

    def modificar_captura(self, clave, dato):
        pos = self.busqueda(clave)
        if pos :
            pos.datos['capturada'] = dato
        
    def modificar_descripcion(self, clave, dato):
        pos = self.busqueda(clave)
        if pos :
            pos.datos['descripcion'] = dato
