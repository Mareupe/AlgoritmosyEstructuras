    ...
    def familia(self, nombre):
        ...
        if (dios != -1):
            ...
            for i in range(aristas.tamanio()):
                ...
                if('padre' in elemento['data']['relacion'] or 'madre' in elemento['data']['relacion']):
                    hijos.append(elemento['destino'])
                elif('hijo' in elemento['data']['relacion']):
                    padre.append(elemento['destino'])
                elif('hermano' in elemento['data']['relacion']):
                    hermano.append(elemento['destino'])
            return padre, hijos, hermano
        else:
